...
batch_size = 256
iter_num = 16
epochs_for_temp_model1 = 64
epochs_for_temp_model2 = 16
epochs_1_for_predition_model = 64
epochs_2_for_predition_model = 16
last_epochs_for_predition_model = 128
...
def main(yaml_filepath, mode):
    ...
    if mode == 'train':
        ...
        temp_weight = temp_model.layers[3].get_weights()[0]
        ...
        temp_model2.fit(x_train,
                        y_train - y_temp_train - temp_biase_weight,
                        epochs=epochs_for_temp_model2, 
                        batch_size=batch_size,
                        validation_data=(x_valid, y_valid - y_temp_valid - temp_biase_weight))
        del temp_model2
        del y_temp_train
        del y_temp_valid
        
        predition_model.layers[1].trainable = False # LSTM
        predition_model.layers[2].trainable = False # conv1d_model
        predition_model.compile(optimizer='adam', loss='mean_squared_error')
        #pms(predition_model)
        predition_model.fit(x_train, y_train,
        ...
        output_image_dir = "output_image/conv1d_plus_lstm_v3.0/patient_"+str(cfg['dataset']['patient_id'])+"/"
