...
def main(yaml_filepath, mode):
    ...
    length = int(24*60/5) + 1
    t = np.linspace(0, 24, num=length)
    output_image_dir = "output_image/pic_1/"
    ...
    for i in range(3):
        ...
        plt.plot(t, nd_glucose_level[:length])
        plt.grid()
        plt.xlabel('Time (hours)')
        plt.ylabel('Glucose Level (mg/dL)')
        plt.title('Patient '+str(patient_true_id))
        plt.savefig(output_image_dir+'Patient '+str(patient_true_id)+'.png')
